...
struct vpn_sm4_ctx {
	spinlock_t vpn_sm4_lock;
	unsigned int sk_enc[32];
	unsigned int sk_dec[32];
};
...
void sm4_setkey_vpn( struct vpn_sm4_ctx *ctx, unsigned char key[16] ) 
{ 
	int i; 
...
int sm4_set_key(struct crypto_tfm *tfm, const u8 *in_key,
		unsigned int key_len)
{
	struct vpn_sm4_ctx *ctx = crypto_tfm_ctx(tfm);

#if DEBUG
	printk(" function: %s ,  line= %d \n", __FUNCTION__, __LINE__);
#endif
	if( key_len != 16) {
		printk(" sm4 set key leng  errr \n");
		return -1;
	}
	sm4_setkey_vpn(ctx, (u8*)in_key); 
...
static void sm4_encrypt(struct crypto_tfm *tfm, u8 *out, const u8 *in)
{
	...
	printk(" function: %s ,  line= %d \n", __FUNCTION__, __LINE__);
#endif
	sm4_one_round(ctx->sk_enc, (u8*)in, out);
}
...
static void sm4_decrypt(struct crypto_tfm *tfm, u8 *out, const u8 *in)
{
	...
	printk(" function: %s ,  line= %d \n", __FUNCTION__, __LINE__);
#endif
	sm4_one_round(ctx->sk_dec, (u8*)in, out);
}
...
static struct crypto_alg sm4_alg = {
	...
	.cra_blocksize		= 16,
	.cra_ctxsize		= sizeof(struct vpn_sm4_ctx),
